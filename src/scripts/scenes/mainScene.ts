...
export default class MainScene extends Scene3D {
  ...
  private redDot: any;
  private sphere: any;
  private enemyAnimations = ['Fall', 'Injure', 'Die']
  private enemyUpdate = true;
  constructor() {
    super({ key: 'MainScene' })
  }
  ...
  create() {
    ...
    this.sphere = new ExtendedObject3D();
    ...
    this.input.on('pointerdown', (pointer) => {
      ...
      this.sphere = this.third.physics.add.sphere(
        {
          name: 'sphere',
          radius: 0.15,
          x: pos.x,
      ...
      this.sphere.body.setBounciness(0.2)

      pos.copy(this.raycaster.ray.direction)
      pos.multiplyScalar(24)

      this.sphere.body.applyForce(pos.x * this.force, pos.y * this.force, pos.z * this.force)
  ...
  addEnemy() {
    ...
    const pos = { x: -15, y: 2.5, z: -24 }
    this.third.load.fbx('/assets/fbx/enemy/Idle.fbx').then(object => {
      this.enemy.add(object);
      this.third.animationMixers.add(this.enemy.anims.mixer);
      this.enemy.anims.play('Idle');
      this.enemy.traverse(child => {
        if (child.isMesh) child.castShadow = child.receiveShadow = true
      })
      this.enemy.scale.set(0.005, 0.005, 0.005)
      ...
      this.enemy.body.setFriction(50)
      this.enemy.body.setCollisionFlags(6);
      const sensor = new ExtendedObject3D();
      sensor.position.setY(-0.9);
      this.third.physics.add.existing(sensor, { mass: 1e-8, shape: 'box', width: 2, height: 2, depth: 0.2 });
      sensor.body.setCollisionFlags(4)
      this.third.physics.add.constraints.lock(this.enemy.body, sensor.body)
      ...
      this.enemyAnimations.forEach(key => {
        if (key === 'Idle') return
        this.third.load.fbx(`/assets/fbx/enemy/${key}.fbx`).then(object => {
  ...
  overLapWithEnemy() {
    this.enemy.body.on.collision((otherObject, event) => {
      if (/sphere/.test(otherObject.name)) {
        if (!otherObject.userData.dead) {
          this.die();

         // this.enemy.userData.dead = true;
  ...
  die() {
   // this.enemy.anims.play('Injure', 300, false)
   this.enemyUpdate = false;
   this.enemy.body.needUpdate = false
    this.time.addEvent({
      delay:0,
      loop:false,
      callback:() => {
       // this.third.physics.destroy(this.enemy);
      //  let tmp = this.enemy.position.clone()
      //  this.enemy.position.set(tmp.x,tmp.y+8,tmp.z)
        this.enemy.anims.play('Die', 2000, false)
